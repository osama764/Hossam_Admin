...
async function addNewRoomInFirebase() {
  ...
  try {
    const response = await fetch('https://hossam-8bf1a-default-rtdb.firebaseio.com/Rooms.json');
    ...
    await fetch(`https://hossam-8bf1a-default-rtdb.firebaseio.com/Rooms/${index}.json`, {
      method: 'PUT',
      headers: {
...
function DisplayData() {
  ...
  request.open('GET', 'https://hossam-8bf1a-default-rtdb.firebaseio.com/Rooms.json');
...
function deleteRoom(roomKey) {
  // إنشاء طلب HTTP
  const request = new XMLHttpRequest();
  request.open('DELETE', `https://hossam-8bf1a-default-rtdb.firebaseio.com/Rooms/${roomKey}.json`);
